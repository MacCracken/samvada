...
| **Authenticity of a reply** | **trusted — enforced by the bus, NOT by samvada** | samvada correlates a reply to its call on `REPLY_SERIAL` alone. It does **not** check `SENDER` (field 7) or `DESTINATION` (field 6), does not learn its own unique name, and uses sequential serials. This is safe on a spec-compliant bus and **only** there: dbus-broker and reference dbus-daemon both implement reply tracking that routes a `METHOD_RETURN`/`ERROR` only to the peer that made the matching call, so a hostile peer's forged reply is dropped before it arrives (verified live — see the audit's *Reply forgery* section). On a bus with permissive policy, without reply tracking, or with an attacker interposed on the `AF_UNIX` socket, a forged reply **would** be accepted unconditionally. Note this is **not** a regression from libsystemd: `sd_bus_call` checks neither field either. Defence-in-depth (a `DESTINATION` check plus a send-time watermark) is scheduled for 1.0.x |
